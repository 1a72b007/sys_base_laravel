<template>
  <div>
    <form class="ac_modal_block" onsubmit="return false">
      ...
      >
        <div class="mb-3">
          <div class="text-value-lg" style="padding-right:15px; padding-left:15px;">{{ block_dto.block_name }}</div>
        </div>
        <div
          :class="inner_block_dto.inner_block_class"
          v-for="(inner_block_dto, inner_block_index) in block_dto.inner_block"
          v-bind:key="inner_block_index"
        >
          <div class="row">
            <div
              :class="field.field_class"
              v-for="(field, field_index) in inner_block_dto.fieldsConfig"
...
export default {
  ...
  methods: {
    submit() {
      this.$validator.validateAll("form_generator").then(async (result) => {
        if (result) {
          this.$emit("save");
        } else {
        }
